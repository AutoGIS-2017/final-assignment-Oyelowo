...
graph_access= ox.graph_from_polygon(bbox_buf, network_type="walk")
ox.save_load.save_gdf_shapefile(graph_access, filename='graph_acc', folder=fp)
ox.save_load.save_graph_shapefile(graph_access, filename='graph_acc', folder=fp, encoding='utf-8')
ox.save_load.save_graphml(graph_access, filename='graph.graphml', folder=fp)

gdf_nodes= gpd.read_file(r'C:\Users\oyeda\Desktop\AUTOGIS\FINAL_ASSIGNMENT\graph_acc\nodes\nodes.shp')
gdf_edges=gpd.read_file(r'C:\Users\oyeda\Desktop\AUTOGIS\FINAL_ASSIGNMENT\graph_acc\edges\edges.shp')
gdf_edges=gpd.GeoDataFrame(gdf_edges, geometry='geometry', crs={'init': 'epsg:4326'})
gdf_edges.crs

gdf_nodes=gpd.GeoDataFrame(gdf_nodes, geometry='geometry', crs={'init': 'epsg:4326'})
gdf_nodes.crs
type(gdf_nodes)
...
ox.save_load.gdfs_to_graph(gdf_nodes, gdf_edges)
ox.save_load.graph_to_gdfs(graph_access, nodes=True, edges=True, node_geometry=True, fill_edge_geometry=True)
